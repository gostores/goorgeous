...
func TestGenerateComment(t *testing.T) {
	p := NewParser(markdown.HtmlRenderer(markdown.HTML_USE_XHTML, "", ""))
...
func TestRenderingFootnotes(t *testing.T) {
	...
	for caseName, tc := range testCases {
		flags := markdown.HTML_USE_XHTML
		flags |= markdown.LIST_ITEM_BEGINNING_OF_LIST
		flags |= markdown.HTML_FOOTNOTE_RETURN_LINKS

		var parameters markdown.HtmlRendererParameters
		parameters.FootnoteReturnLinkContents = "<sup>↩</sup>"

		renderer := markdown.HtmlRendererWithParameters(flags, "", "", parameters)
