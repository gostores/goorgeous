...
func NewParser(renderer markdown.Renderer) *parser {
	p := new(parser)
	p.r = renderer
...
// that the caller wants to use markdown's HTMLRenderer with XHTML
func OrgCommon(input []byte) []byte {
	renderer := markdown.HtmlRenderer(markdown.HTML_USE_XHTML, "", "")
	return OrgOptions(input, renderer)
}

// Org is a convenience name for OrgOptions
func Org(input []byte, renderer markdown.Renderer) []byte {
	return OrgOptions(input, renderer)
}

// OrgOptions takes an org content byte slice and a renderer to use
func OrgOptions(input []byte, renderer markdown.Renderer) []byte {
	...
	for scanner.Scan() {
		...
		case isDefinitionList(data):
			...
			flags := markdown.LIST_TYPE_DEFINITION
			matches := reDefinitionList.FindSubmatch(data)
			flags |= markdown.LIST_TYPE_TERM
	...
	if len(p.notes) > 0 {
		flags := markdown.LIST_ITEM_BEGINNING_OF_LIST
...
func (p *parser) generateHeadline(out *bytes.Buffer, data []byte) {
	...
	headlineID := SanitizedNameCreate(string(data[i:]))
...
func (p *parser) generateList(output *bytes.Buffer, data []byte, listType string) {
	...
	case "ol":
		p.r.List(output, generateList, markdown.LIST_TYPE_ORDERED)
	case "dl":
		p.r.List(output, generateList, markdown.LIST_TYPE_DEFINITION)
	}
}
